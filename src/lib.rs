...
fn create_proxied_request<B>(
    ...
    *request.uri_mut() = forward_uri(forward_url, &request)?;
    ...
    // rewrite host header
    if request.headers().get(HOST).is_some() {
        let uri = Uri::from_str(forward_url).unwrap();
        request
            .headers_mut()
...
// #[cfg(feature = "https")]
pub type HyperClient = Client<hyper_tls::HttpsConnector<HttpConnector<GaiResolver>>, hyper::Body>;

// #[cfg(feature = "https")]
pub fn build_client() -> Client<hyper_tls::HttpsConnector<HttpConnector<GaiResolver>>, hyper::Body>
{
...
// type HyperClient = Client<HttpConnector<GaiResolver>, hyper::Body>;
...
pub async fn call(
    ...
    tracing::debug!("proxy request took={}ms", now.elapsed().as_millis());
